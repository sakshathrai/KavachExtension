...
const data = storage.sync.get("ph-ext-data");
try {
...
class Amazon {
  ...
  GetProductData = () => {
    ...
    if (PageData.includes("buying-options-price-data")) {
      ...
      try {
        TempObj = JSON.parse(
          PageData.split("buying-options-price-data")[1]
            .split("</div>")[0]
            .split(">")[1]
        );
      } catch (error) {}
      ...
      for (let index = 0; index < TempObj.length; index++) {
        if (
          "buyingOptionType" in TempObj[index] &&
          typeof TempObj[index].buyingOptionType === "string"
        ) {
          switch (TempObj[index].buyingOptionType.toLowerCase()) {
            case "lightning_deal":
            case "prime_savings_upsell":
              if (
                "priceAmount" in TempObj[index] &&
                typeof TempObj[index].priceAmount === "number"
              ) {
                try {
                  PriceHistoryData.Lightning = parseFloat(
                    TempObj[index].priceAmount
                  );
                } catch (error) {}
              }
              break;
            ...
            case "new":
              if (
                ...
                try {
                  PriceHistoryData.Price = parseFloat(
                    TempObj[index].priceAmount
                  );
                } catch (error) {}
              }
    ...
    if (
      PageData.includes(" coupon applied ") ||
      PageData.includes(" voucher applied ")
    ) {
      if (PageData.includes(" coupon applied ")) {
        PriceHistoryData.Coupon = GetArrayLastValue(
          PageData.split(" coupon applied ")[0].split(">")
        )
          .replace(/\&.*?\;/g, "")
          .trim();
      } else if (PageData.includes(" voucher applied ")) {
        PriceHistoryData.Coupon = GetArrayLastValue(
          PageData.split(" voucher applied ")[0].split(">")
        )
          .replace(/\&.*?\;/g, "")
          .trim();
      }
    ...
    if (PageData.includes("config.isPrimeMember',")) {
      PriceHistoryData.IsMember =
        PageData.split("config.isPrimeMember',")[1].split(")")[0].trim() ==
        "true"
          ? 1
          : 0;
    }
  ...
  AdditionalFeatures = async (Product) => {
    try {
      let LowestAmount = parseFloat(
        document
          .getElementsByClassName("olp-link-widget")[0]
          .innerText.replaceAll(/\n+/g, " ")
          .match(/₹[\d,]+\.{0,1}\d*/)[0]
          .replaceAll(",", "")
          .replace("₹", "")
      );
      let FlagDisplayButton =
        Product.Lightning && Product.Lightning < LowestAmount
          ? false
          : Product.Price && Product.Price > LowestAmount
          ? true
          : false;

      if (FlagDisplayButton) {
        let ButtenText =
          "Available at ₹" + LowestAmount + " by other seller. Check Now ⏩";
        let TempElement = CreatePriceElement(
          "SeeAllPriceButton",
          { BG: "#FF8F00", Color: "#0F1111" },
          ButtenText,
          {
            ...
            Url:
              ...
              "?phasource=extension&phamethod=allseller&phastore=" +
              this.Constants.Domain +
              "&phaproduct=" +
              Product.Code,
          }
        ...
        TempElement.addEventListener("click", () => {
          CONSTANTS.GenerateGAEvent("product_redirect", {
            product_code: Product.Code,
            store_domain: this.Constants.Domain,
            price: "" + LowestAmount,
            redirect_type: "all_price",
            click_source: "see_all_price",
        ...
        if (this.PriceContainer && this.PriceContainer != null) {
          ...
          this.GetElementAfterEnd().insertAdjacentElement(
            "afterend",
            TempElement
          );
        }
...
class Flipkart {
  ...
  GetProductData = () => {
    ...
    let PageData = document.documentElement.innerHTML.toString();
    PriceHistoryData.Code = new URL(window.location.href).searchParams.get(
      "pid"
    );
    if (
      PriceHistoryData.Code != null &&
      PriceHistoryData.Code.match(/[0-9A-Za-z]{15}/)
    ) {
    } else if (PageData.includes("sellers?pid=")) {
      PriceHistoryData.Code = PageData.split("sellers?pid=")[1]
        .split("&")[0]
        .trim();
    } else {
      return PriceHistoryData;
    ...
    if (PageData.includes('id="jsonLD"')) {
      try {
        let TempData = PageData.split('id="jsonLD"')[1]
          .split("</script")[0]
          .splice(">")
          .splice(1)
          .join(">");
        for (let index = 1; index < TempData.length; index++) {
          try {
            ...
            for (let index1 = 0; index1 < TempData1.length; index1++) {
              if (
                TempData1[index1] &&
                "offers" in TempData1[index1] &&
                TempData1[index1].offers != null &&
                typeof TempData1[index1].offers === "object" &&
                "price" in TempData1[index1].offers.price &&
                TempData1[index1].offers.price != null &&
                typeof TempData1[index1].offers.price === "number"
              ) {
    ...
    if (TempFlag && PageData.includes("price-info-icon")) {
      ...
      for (let index = 0; index < TempData1.length; index++) {
        if (
          "Price" in PriceHistoryData &&
          PriceHistoryData.Price != null &&
          typeof PriceHistoryData.Price === "number"
        ) {
    ...
    if (PageData.includes('id="pincodeInputId" value="')) {
      PriceHistoryData.Pincode = PageData.split(
        'id="pincodeInputId" value="'
      )[1].split('"')[0];
    }
  ...
  AdditionalFeatures = async (Product) => {
    ...
    Iframe.src =
      "https://www.flipkart.com/sellers?data=" +
      new Date().getTime() +
      "&pid=" +
      Product.Code;
    ...
    Iframe.addEventListener("load", () => {
      setTimeout(() => {
        let PageData =
          Iframe.contentDocument.documentElement.innerHTML.toString();
        if (PageData.includes("<span>Delivery</span>")) {
          let Sellers = [];
          try {
            let RelatedContentBlocks = PageData.split(
              "<span>Delivery</span>"
            )[1]
              .split("<footer")[0]
              .split('col-3-12">')
            ...
            for (let i = 1; i < RelatedContentBlocks.length; i = i + 2) {
              let Seller = {};
              try {
                Seller.Name = RelatedContentBlocks[i - 1]
                  .split("</span></div>")[0]
                  .split("<span>")[1]
                  .trim();
              } catch (e) {}
              try {
                Seller.Rating = parseFloat(
                  GetArrayLastValue(
                    RelatedContentBlocks[i - 1].split("<img")[0].split(">")
                  ).trim()
                );
              } catch (e) {}
              try {
              ...
              try {
                let AllPrices = RelatedContentBlocks[i]
                  .split(">₹")
                  .splice(1)
                  .map((blocks) => {
                    return blocks.split("<")[0].replaceAll(",", "");
                  });
                AllPrices = AllPrices.filter(
                  (oneprice) =>
                    !(
                      oneprice.toLowerCase().includes("off") ||
                      oneprice.toLowerCase().includes(" + ")
                    )
                );
          ...
          if (Sellers.length > 1) {
            if ("Price" in Product && Product.Price > Sellers[0].Price) {
              let TempElement = CreatePriceElement(
                "SeeAllPriceButton",
                { BG: "#fb641b", Color: "#f0f0f0" },
                "Available at ₹" +
                  Sellers[0].Price +
                  " by other seller. Check Now ⏩ ",
                {
                  Url:
                    CONSTANTS.Buy +
                    "/" +
              ...
              if (this.PriceContainer && this.PriceContainer != null) {
                this.PriceContainer.insertAdjacentElement(
                  "afterend",
                  TempElement
                );
              } else {
...
const CreatePriceElement = (Type, Color, Name, ExtraObj) => {
  switch (Type) {
    case "priceElement":
      ...
      PriceElement.innerHTML = `<span style=\"font-size:smaller;\">${Name}: </span>${
        ExtraObj.InLine ? "" : "<br>"
      }<span style=\"font-weight:bolder;\">₹${ExtraObj.Price}</span>`;
      return PriceElement;
      break;
    ...
    case "SeeAllPriceButton":
      ...
      let TempStyle =
        ...
        ";color: " +
        Color.Color +
        ";padding: 0.25rem 1rem;border-radius:0.25rem;text-decoration:none;box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.2);font-weight:500;";
...
const UpdatePrices = (Prices, ele) => {
  ...
  if (
    Prices.highest ||
    Prices.lowest ||
    (Prices.total && Prices.count) ||
    Prices.lowestoffer
  ) {
    PriceContainer = document.createElement("div");
    PriceContainer.className = CONSTANTS.RemoveElementsClassName;
    PriceContainer.style =
      "background-color: #00000010;border-radius: 0.25rem;margin-bottom: 0.5rem;box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.2);text-align: center;display:table;";
    if (Prices.highest) {
      PriceContainer.appendChild(
        CreatePriceElement("priceElement", { Color: "#ff534b" }, "Highest", {
          Price: parseInt(Prices.highest),
    ...
    if (Prices.total && Prices.count) {
      PriceContainer.appendChild(
        CreatePriceElement("priceElement", { Color: "#555555" }, "Average", {
          Price: parseInt(Prices.total / Prices.count),
          Display: "inline-block",
          InLine: false,
        })
      );
    ...
    if (Prices.lowest) {
      ...
      PriceContainer.appendChild(
        CreatePriceElement("priceElement", { Color: "#17a2b8" }, "Lowest", {
          Price: parseInt(Prices.lowest),
          Display: "inline-block",
          InLine: false,
        })
      );
        ...
        if (Prices.lowest > Prices.lowestoffer) {
          PriceContainer.appendChild(
            CreatePriceElement(
              ...
              "Lowest Offer Price",
              {
                Price: parseInt(Prices.lowestoffer),
                Display: "block",
                InLine: true,
              }
            )
  ...
  if ("code" in Prices && Prices.code.length > 0) {
    if (
      !(
        ExtensionConfigs &&
        ExtensionConfigs["enable-floating-button"] &&
        ExtensionConfigs["enable-floating-button"] === false
      )
    ) {
      let ChartButton = document.createElement("div");
      ChartButton.className = CONSTANTS.RemoveElementsClassName;
      ChartButton.style =
        "position: fixed; z-index: 1001; text-align: center; background-color: #021542; padding: 0rem 0.5rem 0.2rem 0.5rem; border-radius: 0.5rem; cursor: pointer;" +
        CONSTANTS.ExtensionPosition(
          ExtensionConfigs &&
            ExtensionConfigs["floating-button-location"] &&
            ExtensionConfigs["floating-button-location"].length > 0
            ? ExtensionConfigs["floating-button-location"]
            : ""
        );
      ...
      ChartButtonImage.src = runtime.getURL("assets/images/icon-128x128.png");
      ...
      ChartButtonText.style =
        "font-size: 0.65rem; color: #ffffff; display: block; margin-top: -0.6rem;";
    ...
    if (PriceContainer != null) {
      ...
      ChartSmallImageButton.src = runtime.getURL(
        "assets/images/icon-128x128.png"
      );
      ChartSmallImageButton.title = "Price History Chart";
      ChartSmallImageButton.style =
        "float:left;height:1.75rem;border-radius:50%;cursor: pointer;";
      PriceContainer.insertBefore(
        ChartSmallImageButton,
        PriceContainer.firstChild
      );
      ChartSmallImageButton.addEventListener("click", OpenChart);
    }
    ...
    ChartContainer.style =
      "position: fixed; top: 0px; bottom: 0px; right: 0px; left: 0px; text-align: center;margin: auto;background-color: #00000055; z-index:1002;display:none;";
    ...
    CONSTANTS.GenerateGAEvent("product_graph_loaded", {
      product_code: CurrentChartUrl,
      engagement_time_msec: "100",
    });
    document.body.appendChild(ChartContainer);

    ChartContainer.getElementsByClassName(
      "pricehistory-chart-close"
    )[0].addEventListener("click", (event) => {
      document.getElementById("pricihistory-embedded-chart").style.display =
        "none";
    });
...
const OpenChart = () => {
  ...
  if (ChartContainer) {
    CONSTANTS.GenerateGAEvent("product_graph_visible", {
      product_code: CurrentChartUrl ? CurrentChartUrl : "",
      engagement_time_msec: "100",
    });
...
const removeElements = () => {
  let elementsToRemove = document.querySelectorAll(
    "." + CONSTANTS.RemoveElementsClassName
  );
...
const StartExecution = () => {
  ...
  if (IsProductPage) {
    ...
    if (GetProductData.Status) {
      ...
      CONSTANTS.GenerateGAEvent("product_page_opened", {
        product_code: GetProductData.Code ? GetProductData.Code : "",
        price: GetProductData.Price ? "" + GetProductData.Price : "",
        store_promotion: GetProductData.Coupon ? GetProductData.Coupon : "",
        store_deal: GetProductData.Lightning ? GetProductData.Lightning : "",
        store_domain: GetProductData.Store ? GetProductData.Store : "",
        user_type: GetProductData.IsMember ? "yes" : "no",
        pincode: GetProductData.Pincode ? GetProductData.Pincode : "",
        product_url: PageURL.href.substring(0, 99),
        product_name: GetProductData.Name
          ? GetProductData.Name.substring(0, 99)
          : "",
        engagement_time_msec: "100",
      });
      ...
      runtime.sendMessage(
        ...
        (response) => {
          if (response.status) {
            ...
            Obj.PriceContainer = UpdatePrices(
              response.data,
              Obj.GetElementAfterEnd()
            );
            if (response.data && response.data.status && response.data.status) {
              CONSTANTS.GenerateGAEvent("product_price_fetch", {
                product_code: GetProductData.Code ? GetProductData.Code : "",
                error: "false",
                valid: "true",
                message: "Data found: " + JSON.stringify(response.data),
                engagement_time_msec: "100",
              });
            } else {
              CONSTANTS.GenerateGAEvent("product_price_fetch", {
                ...
                error: "false",
                valid: "false",
                message: "Data Not found: " + JSON.stringify(response),
                engagement_time_msec: "100",
              });
            }
          } else {
            CONSTANTS.GenerateGAEvent("product_price_fetch", {
              ...
              error: "true",
              valid: "false",
              message: "Fetch Error: " + JSON.stringify(response.error),
              engagement_time_msec: "100",
            });
          }
          if (
